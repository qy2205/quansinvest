...
def forward_return_summary_statistics(
    statistics_results,
    name="nasdaq",
    display_dataframe=True,
):
    next_day_returns = pd.Series(
        [each_period[-1].get("period_return", 0) for each_period in statistics_results])
    ...
    if display_dataframe:
        display(statistics_results[max_return_index][1])
    ...
    if display_dataframe:
        display(statistics_results[min_return_index][1])


def forward_return_eval_metrics(
    statistics_results,
    ...
    for period_dfs in statistics_results:
        for eval_metric in eval_metrics:
            # sequential patterns
    ...
    if MaxLoss.name in eval_results_df.columns:
        ...
        if display_dataframe:
            display(statistics_results[max_loss_index][1])
        ...
        if display_dataframe:
            display(statistics_results[min_loss_index][1])
    ...
    if MaxProfit.name in eval_results_df.columns:
        ...
        if display_dataframe:
            display(statistics_results[max_profit_index][1])
        ...
        if display_dataframe:
            display(statistics_results[min_profit_index][1])
